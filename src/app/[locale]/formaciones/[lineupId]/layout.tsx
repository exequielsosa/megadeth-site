...
export async function generateMetadata({
  ...
  params: { locale: string; lineupId: string };
}): Promise<Metadata> {
  const { locale, lineupId } = params;
  const lineups: LineupFormation[] = lineupsData.lineups;
  ...
  const title = lineup.title[locale as "es" | "en"] || lineup.title.es;
  const description =
    lineup.description[locale as "es" | "en"] || lineup.description.es;
