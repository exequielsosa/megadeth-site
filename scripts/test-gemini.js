...
console.log(
  `✅ GEMINI_API_KEY encontrada: ${GEMINI_API_KEY.substring(0, 20)}...\n`,
);
...
try {
  ...
  if (availableModels.length === 0) {
    console.log(
      "   ⚠️  No se encontraron modelos con soporte para generateContent",
    );
    process.exit(1);
  }
  ...
  const prompt =
    "Responde únicamente con la palabra 'OK' si puedes leer este mensaje.";
  ...
  if (error.message.includes("403")) {
    console.log("\n🔧 Posibles soluciones:");
    console.log(
      "   1. Verifica que la API Key sea correcta (copia/pega sin espacios)",
    );
    console.log(
      "   2. Ve a https://aistudio.google.com/app/apikey y genera una nueva key",
    );
    console.log(
      '   3. Asegúrate de habilitar "Generative Language API" en Google Cloud Console',
    );
    console.log("   4. Si es nueva, espera 1-2 minutos para que se active\n");
  } else if (error.message.includes("404")) {
    console.log("\n🔧 El modelo especificado no existe.");
    console.log(
      "   Ejecuta este script de nuevo para ver los modelos disponibles.\n",
    );
  }
